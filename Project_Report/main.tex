...
\Large{\textbf{Prathma Jain}  \Large{(Roll no. 39)}} \\
 \Large{\textbf{Purva Patil} \Large{(Roll no. 42)}}
\\
\\
 \Large{\textbf{Nixson Arumugam} \Large{(Roll no. 43)}}
\\
 \Large{\textbf{Abishek Jayaraj} \Large{(44)}}\\
\vspace{0.4cm}
...
\begin{longtable}{|p{2.5cm}|l|l|l|c|p{4.5cm}|}
    ...
    \caption{Pin specifications for the BLE Baseband Processor Core.}
    \label{tab:ble_pins} \\
    ...
    % --- YOUR TABLE DATA ---
    BLE Baseband Processor Core & VDD, VSS & Input & Power & 1 & Power (1.8V) and Ground connections. \\ \hline
...
The Cyclic Redundancy Check (CRC) is a critical component of the digital baseband, responsible for detecting transmission errors in the BLE packet. This section details the mathematical and hardware derivation of the 24-bit CRC used in the Physical Layer.

\subsubsection{Mathematical Model in GF(2)}
The CRC algorithm treats the input bitstream as a polynomial $M(x)$ and performs division over the Galois Field $GF(2)$. The generator polynomial $P(x)$ specified by the Bluetooth Core Specification is:
\begin{equation}
P(x) = x^{24} + x^{10} + x^9 + x^6 + x^4 + x^3 + x + 1
\end{equation}
In $GF(2)$, addition and subtraction are replaced by the XOR operation, and there are no carries. The CRC generation is defined as the remainder $R(x)$ of the following division:
\begin{equation}
R(x) = [M(x) \cdot x^{24}] \pmod{P(x)}
\end{equation}

\subsubsection{Step-by-Step LFSR Hardware Mapping}
To implement this division efficiently in RTL, a Linear Feedback Shift Register (LFSR) in the Fibonacci configuration is utilized. The hardware mapping follows these logical steps:
\begin{enumerate}
    \item \textbf{Register Chain:} 24 D-type flip-flops are connected in series to store the current state of the remainder.
    \item \textbf{Feedback Logic:} XOR gates are placed at the feedback taps defined by the coefficients of $P(x)$.
    \item \textbf{Serial Processing:} The payload bits are shifted into the LFSR bit-serially. Each shift represents one step of the long division process.
    \item \textbf{Initialization:} Prior to processing, the LFSR is pre-set to a 24-bit \textit{CRC Initialization Vector} (Seed) to ensure channel-specific synchronization.
\end{enumerate}

\subsubsection{Significance in RTL Design}
The LFSR-based CRC architecture provides a zero-latency calculation of the checksum. By calculating the CRC as the packet is being transmitted or received, the baseband processor can append or verify the checksum without buffering the entire payload. This bit-serial approach minimizes the gate count and power consumption of the digital logic, which is a primary design constraint for Bluetooth Low Energy hardware.

\subsection{Data Whitening using Galois LFSR}

Data whitening is a fundamental randomization process used in the BLE Physical Layer to ensure spectral stability and facilitate receiver synchronization.

\subsubsection{ Problem Statement: DC Bias and Clock Drift}
Standard digital payloads often contain repetitive sequences of "0"s or "1"s. In the RF domain, these sequences lead to a lack of transitions, causing two primary issues:
\begin{enumerate}
    \item \textbf{DC Offset:} Continuous identical bits create a non-zero average voltage in the baseband, complicating receiver threshold detection.
    \item \textbf{Clock Recovery:} Receivers utilize bit transitions to maintain timing synchronization. A lack of transitions leads to sampling jitter and increased Bit Error Rate (BER).
\end{enumerate}

\subsubsection{The Galois LFSR Mathematical Model}
The whitening process utilizes a 7-bit Linear Feedback Shift Register (LFSR) configured in a Galois arrangement. The pseudo-random sequence is generated based on the following characteristic polynomial:
\begin{equation}
P(x) = x^7 + x^4 + 1
\end{equation}
This polynomial produces a maximal-length sequence (m-sequence) of $127$ bits. The whitening is achieved by performing a bit-wise XOR operation between the raw payload $D_{in}$ and the output of the LFSR $W_{out}$:
\begin{equation}
D_{whitened} = D_{in} \oplus W_{out}
\end{equation}

\subsubsection{Hardware Architecture and State Transitions}
In a Galois LFSR, the feedback logic is distributed between the shift registers. For the polynomial $x^7 + x^4 + 1$, the state transition of the $i$-th register $S_i$ at time $n+1$ is defined as:
\begin{equation}
S_4(n+1) = S_3(n) \oplus S_6(n)
\end{equation}
All other registers $S_i$ (where $i \neq 4$ and $i \neq 0$) simply perform a shift operation $S_i(n+1) = S_{i-1}(n)$, while $S_0(n+1) = S_6(n)$. 

\subsubsection{Significance in RTL and System Design}
The Galois configuration is chosen for its superior timing characteristics in RTL synthesis, as it minimizes the combinational logic depth between registers. Furthermore, the whitening module is initialized at the start of every packet with a 7-bit seed based on the channel index. This ensures that the physical signal varies across different frequencies, providing a form of processing gain that improves the robustness of the BLE link in crowded environments.
...
\section{3.4 Gaussian Frequency Shift Keying (GFSK) Modulation}

Gaussian Frequency Shift Keying (GFSK) serves as the fundamental modulation scheme for the Bluetooth Low Energy (BLE) physical layer. It is a constant-envelope modulation technique that optimizes spectral efficiency through pre-modulation pulse shaping.

\subsection{Principles of GFSK Operation}
GFSK modulation functions by shifting the carrier frequency $f_c$ in response to digital data. Unlike standard Frequency Shift Keying (FSK), where the frequency transitions are abrupt, GFSK utilizes a Gaussian low-pass filter to smooth the NRZ (Non-Return-to-Zero) bitstream. This filtering process suppresses the high-frequency harmonics of the square-wave pulses, thereby constraining the signal within the allocated 1 MHz channel bandwidth and minimizing Adjacent Channel Interference (ACI).

\subsection{The Gaussian Shaping Filter}
The performance of the GFSK modulator is governed by the Bandwidth-Time ($BT$) product. Per the BLE specification, a $BT$ of 0.5 is employed. This value ensures a critical balance between:
\begin{enumerate}
    \item \textbf{Occupied Bandwidth:} Reducing the power spectral density (PSD) sidelobes to satisfy regulatory masks.
    \item \textbf{Intersymbol Interference (ISI):} Maintaining sufficient pulse integrity to allow for low-complexity non-coherent detection at the receiver.
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{Gaussian_curve.jpg}
\caption{A Gaussian curve}
\label{fig:Gaussian curve}
\end{figure}

\subsection{Significance in RTL Implementation}
The GFSK module is implemented as a digital baseband processor that drives a Numerically Controlled Oscillator (NCO). By utilizing a Look-Up Table (LUT) for the Gaussian-shaped pulses and a phase accumulator for integration, the hardware achieves a precise, phase-continuous output. This architecture is highly efficient for ASIC and FPGA implementations, providing the necessary spectral containment for BLE while minimizing power consumption.
...
\section{Mathematical Derivation of the GFSK Shaping Pulse }


In a digital circuit, a bit is represented by a square wave. However, the "infinitely sharp" corners of a square wave require infinite bandwidth, which would cause interference. To ensure reduced bandwidth and continuous phase for Bluetooth Low Energy (BLE), we "round off" these transitions using a \textbf{Gaussian Filter} through a process called \textbf{Convolution}.

\subsection{Defining the Gaussian Filter in Frequency}
We define the filter in the Frequency Domain ($f$) as a bell curve:
\begin{equation}
H(f) = e^{-af^2}
\end{equation}
To find the constant $a$, we use the \textbf{3dB Bandwidth ($B$)}. At frequency $f = B$, the voltage drops to $1/\sqrt{2}$:
\begin{enumerate}
    \item $e^{-aB^2} = \frac{1}{\sqrt{2}}$
    \item $-aB^2 = \ln(1) - \ln(\sqrt{2}) = -\frac{1}{2}\ln(2)$
    \item $a = \frac{\ln(2)}{2B^2}$
\end{enumerate}
The final Frequency Response is:
\begin{equation}
H(f) = \exp\left( -\frac{\ln(2)}{2} \left( \frac{f}{B} \right)^2 \right)
\end{equation}

\subsection{The Inverse Fourier Transform}
To implement this in RTL, we need the \textbf{Impulse Response} $h(t)$ in the Time Domain. We use the Inverse Fourier Transform:
\begin{equation}
h(t) = \int_{-\infty}^{\infty} H(f) e^{j2\pi ft} df = \int_{-\infty}^{\infty} e^{-af^2} e^{j2\pi ft} df
\end{equation}
By completing the square in the exponent and solving the Gaussian integral, we obtain:
\begin{equation}
h(t) = \frac{\sqrt{\pi}}{\alpha} \exp\left( -\frac{\pi^2 t^2}{\alpha^2} \right) \quad \text{where} \quad \alpha = \sqrt{\frac{\ln(2)}{2}} \frac{1}{B}
\end{equation}

\subsection{Convolution and the Error Function ($\text{erf}$)}
The shaping pulse $g(t)$ is the convolution of a rectangular bit pulse $p(t)$ (of duration $T_b$) with the filter $h(t)$:
\begin{equation}
g(t) = p(t) * h(t) = \frac{1}{T_b} \int_{t-T_b/2}^{t+T_b/2} h(\tau) d\tau
\end{equation}
Since the integral of a Gaussian has no elementary solution, we use the \textbf{Error Function}, defined as $\text{erf}(x) = \frac{2}{\sqrt{\pi}} \int_0^x e^{-u^2} du$. After substitution and introducing the \textbf{$BT$ Product}, the final engineering form for the BLE Baseband is:
...
\subsection{Hardware Significance (RTL Implementation)}
For the BLE project, calculating $\text{erf}$ in real-time is computationally expensive for hardware. Instead:
\begin{itemize}
    \item We fix $BT = 0.5$ and $T_b = 1\,\mu s$ as per the standard.
    \item We pre-calculate values of $g(t)$ and store them in a \textbf{Look-Up Table (LUT)}.
    \item The RTL logic reads the LUT to drive the GFSK modulator, ensuring a perfect smooth frequency shift.
\end{itemize}


\section{Phase Accumulator Calculations}
...
\begin{itemize}
    ...
    \item \textbf{Modulation Index ($h$):} For BLE, $h = 0.5$.
    \item \textbf{Bit Duration ($T_b$):} The time for one bit ($1 \, \mu s$ for 1 Mbps PHY).
    \item \textbf{Instantaneous Phase ($\theta(t)$):} The value used to drive the Sine/Cosine generator.
\end{itemize}
...
\begin{equation}
\Delta f = \frac{0.5}{2 \times 10^{-6}} = 250 \, \text{kHz}
\end{equation}

\subsection{Phase Integral Derivation}
The Baseband Processor generates a phase $\theta(t)$ that is the integral of the frequency. Given a sequence of bits $a_i$ and the shaping pulse $g(t)$, the instantaneous frequency deviation is:
\begin{equation}
...
\end{equation}
This confirms that the total phase is the cumulative sum of the areas under the shaped pulses, ensuring a smooth transition between symbols.

\subsection{RTL Significance: From Calculus to Hardware}
In the RTL implementation, this mathematical model is mapped to a \textbf{Phase Accumulator} within a Numerically Controlled Oscillator (NCO):
\begin{enumerate}
    \item \textbf{Continuous vs. Discrete:} The integral is replaced by a digital summation: $\theta[n] = \theta[n-1] + f_{word}[n]$.
    \item \textbf{Phase Continuity:} The accumulator ensures that when a bit changes (e.g., $+1$ to $-1$), the phase starts from its last value rather than resetting. This prevents "Phase Jumps" that would cause spectral splatter.
    \item \textbf{Hardware Efficiency:} The integral of the shaping pulse is pre-calculated and stored in a Look-Up Table (LUT), reducing the FPGA/ASIC workload to simple addition.
\end{enumerate}

\section{Implementation Parameters and NCO Calibration}

To translate the theoretical GFSK model into a functional RTL design, the system parameters must be mapped to the hardware clocking constraints and Numerically Controlled Oscillator (NCO) bit-widths.

\subsection{Clock Synthesis and PLL Scaling}
The design utilizes a Phase-Locked Loop (PLL) to multiply the input reference frequency, providing sufficient timing resolution for the GFSK shaping pulse.
\begin{itemize}
    ...
    \end{equation}
\end{itemize}
In the Verilog module \texttt{tx\_modulator\_clocks.v}, this is parameterized as \texttt{PLL\_MULT = 20}.

\subsection{Samples Per Symbol (SPS)}
The SPS defines the discretization of the Gaussian ramp. A higher SPS results in a smoother frequency transition and lower spectral leakage.
\begin{itemize}
    \item \textbf{Data Rate ($R$):} 20 Mbps
    \item \textbf{SPS Calculation:}
    \begin{equation}
    SPS = \frac{F_{int}}{R} = \frac{400 \text{ MHz}}{20 \text{ Mbps}} = 20 \text{ samples/bit}
    \end{equation}
\end{itemize}

\subsection{NCO Tuning Word Calculation}
The NCO employs an 8-bit phase accumulator ($N=256$). The frequency control word (FCW or $M$) determines the phase step size per clock cycle:
\begin{equation}
M = \frac{F_{target} \times 2^N}{F_{int}}
\end{equation}

\subsubsection{Carrier and Deviation Mapping}
Using the 400 MHz system clock, the tuning words for the center carrier and the frequency shift are calculated as:
\begin{itemize}
    \item \textbf{Center Carrier (100 MHz):} $M_{center} = \frac{100 \times 256}{400} = 64$ ($8'h40$)
    \item \textbf{Frequency Deviation (10 MHz):} $M_{dev} = \frac{10 \times 256}{400} \approx 6$
\end{itemize}

Consequently, the modulated output frequencies are represented by the following digital values:
\begin{itemize}
    \item \textbf{Logic '1' (110 MHz):} $64 + 6 = 70$
    \item \textbf{Logic '0' (90 MHz):} $64 - 6 = 58$
\end{itemize}

\subsection{Simulation and Verification Results}
The simulation timing is scaled to match the 400 MHz physical behavior. With a period $T = 2.5 \text{ ns}$, the simulator toggle rate is set to $1.25 \text{ ns}$. The following table summarizes the expected versus observed values during GTKWave verification.

\begin{table}[h]
\centering
\caption{GTKWave Verification Summary}
\vspace{0.5em}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Signal} & \textbf{Expected Frequency} & \textbf{Expected Period} \\ \midrule
\texttt{ext\_clk}   & 20 MHz                & 50 ns \\
\texttt{clk\_fast}  & 400 MHz               & 2.5 ns \\
\texttt{rf\_out} (Logic 1) & 106.4 MHz$^*$  & $\approx$ 9.4 ns \\
\texttt{rf\_out} (Logic 0) & 93.6 MHz$^*$   & $\approx$ 10.6 ns \\ \bottomrule
\end{tabular}
\end{table}
\section{Summary}
