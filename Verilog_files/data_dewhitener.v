/**
 * @module data_dewhitener
 * @brief Inverse of Data Whitener - Descrambles whitened BLE data
 * @details Uses identical LFSR to the whitener to recover original bit stream
 */
module data_dewhitener(
    clk,
    rst_n,
    lfsr_load,          // Pulse to load the channel value
    enable,
    channel_index,
    data_in,            // Whitened data (from demodulator)
    dewhitened_data_out // Original data (descrambled)
);
    ...
    input        data_in;         // Whitened data input
    output       dewhitened_data_out; // Dewhitened output (1-bit)
    ...
    wire         lfsr_out_bit;    // The LSB of the LFSR, used for dewhitening
    ...
    // 3. Define the output bit (the LSB) - same as whitener
    assign lfsr_out_bit = lfsr[0];

    // 4. Define the final dewhitened output (XOR recovers original data)
    assign dewhitened_data_out = data_in ^ lfsr_out_bit;
